...
## সি শার্প কর্নার (C# Corner) থেকে (অতিরিক্ত)

### ১৩২. Node.js কিভাবে ইন্সটল ও ভেরিফাই করব?
**উত্তর:** nodejs.org থেকে ইন্সটলার ডাউনলোড করে ইন্সটল করো। ইন্সটলেশনে **node** (জাভাস্ক্রিপ্ট ইঞ্জিন চালানোর জন্য), **npm** (প্যাকেজ ম্যানেজার) ও **node_modules** (লোকাল প্যাকেজ ফোল্ডার) থাকে। ভেরিফাই: কমান্ড প্রম্পটে `node` লিখে এন্টার করলে Node প্রম্পট আসে, সেখানে `console.log("test");` চালালে আউটপুট দেখাবে। `node -v` ও `npm -v` দিয়ে ভার্সন চেক করো।

---

### ১৩৩. JavaScript/Node এ Closure কি?
**উত্তর:** Closure = একটা ফাংশন যেটা আরেকটা স্কোপের ভিতরে ডিফাইন এবং বাইরের স্কোপের ভেরিয়েবল অ্যাক্সেস করতে পারে। ফাংশন রিটার্ন করার পরও সেই আউটার ভেরিয়েবলগুলো “জীবিত” থাকে। তিনটা চেইন: নিজের স্কোপ, বাইরের ফাংশনের ভেরিয়েবল, গ্লোবাল। উদাহরণ: ফাংশন থেকে ফাংশন রিটার্ন করলে ভিতরের ফাংশন বাইরের ভেরিয়েবল মনে রাখে।

---

### ১৩৪. Process Object কি?
**উত্তর:** Node এ **process** গ্লোবাল অবজেক্ট; যেকোনো জায়গা থেকে অ্যাক্সেস করা যায়, EventEmitter এর ইনস্ট্যান্স। **stdio:** `process.stdin` (পড়া), `process.stdout` (লেখা), `process.stderr` (এরর)। আরও: `process.env`, `process.argv`, `process.exit()`, `process.cwd()`, `process.title`। প্ল্যাটফর্ম, এনভায়রনমেন্ট ও প্রসেস কন্ট্রোলে কাজে লাগে।

---

### ১৩৫. Colors মডিউল কি?
**উত্তর:** NPM প্যাকেজ (`npm install colors`) যেটা কনসোলে রঙিন টেক্সট আউটপুট দেয়। স্ট্রিং প্রোটোটাইপ এক্সটেন্ড করে: `"hello".green`, `"text".underline.red`, `"rainbow".rainbow`। লগ আর ডিবাগ মেসেজ আলাদা করতে ব্যবহার হয়। বিল্ট-ইন মডিউল নয়।

---

### ১৩৬. Node.js vs PHP?
**উত্তর:** **Node:** ইভেন্ট-চালিত, নন-ব্লকিং I/O, এক থ্রেডে অনেক কানেকশন; রিয়েল-টাইম ও I/O-হেভি অ্যাপের জন্য। **PHP:** রিকোয়েস্ট প্রতি আলাদা প্রসেস/থ্রেড, ব্লকিং; ট্র্যাডিশনাল ওয়েব পেজ ও CMS এর জন্য। Node জাভাস্ক্রিপ্ট, PHP আলাদা ল্যাঙ্গুয়েজ। পারফরম্যান্স ও কনকারেন্সি মডেল আলাদা।

---

### ১৩৭. Request ও Cheerio প্যাকেজ কি কাজে লাগে?
**উত্তর:** **Request** (অবসোলেট; এখন **axios** বা **node-fetch** বেশি ব্যবহার) — অন্য সার্ভার থেকে HTTP রিকোয়েস্ট, কুকি/রিডাইরেক্ট হ্যান্ডেল। **Cheerio** — HTML পার্স করে jQuery-র মতো সিনট্যাক্সে এলিমেন্ট সিলেক্ট ও ম্যানিপুলেট; ওয়েব স্ক্র্যাপিংয়ে ব্যবহৃত। Request দিয়ে পেজ লোড, Cheerio দিয়ে ডাটা বের করা।

---

### ১৩৮. Karma ও Jasmine কি?
**উত্তর:** **Jasmine** — BDD স্টাইলের JavaScript টেস্ট ফ্রেমওয়ার্ক; `describe`, `it`, `beforeEach`, `expect` দিয়ে ইউনিট টেস্ট লেখা। **Karma** — টেস্ট রানার; জেসমাইন/মোচা দিয়ে লেখা টেস্ট রিয়েল ব্রাউজারে চালায় (CLI থেকে)। Karma টেস্ট ফ্রেমওয়ার্ক নয়, টেস্ট রান করার টুল। অ্যাংগুলার প্রজেক্টে প্রায়ই একসাথে ব্যবহার হয়।

---

### ১৩৯. Grunt কি?
**উত্তর:** JavaScript টাস্ক রানার (কমান্ড-লাইন টুল), Node.js এ চলে। বিল্ড, মিনিফাই, কম্পাইল, টেস্ট রান, ডিপ্লয় ইত্যাদি অটোমেট করতে প্লাগইন ব্যবহার করা হয়। `npm install grunt --save-dev`, তারপর `Gruntfile.js` এ টাস্ক ডিফাইন। ওয়েবপ্যাক/গাল্পের বিকল্প (পুরনো প্রজেক্টে এখনও দেখা যায়)।

---

### ১৪০. কোন কোন কোম্পানি Node.js ব্যবহার করে?
**উত্তর:** Netflix, LinkedIn, PayPal, Uber, NASA, eBay, Microsoft, Yahoo, Trello, Walmart ইত্যাদি। ওয়েব API, রিয়েল-টাইম অ্যাপ, মাইক্রোসার্ভিস, চ্যাট, স্ট্রিমিং—এ ধরনের জায়গায় Node ব্যবহার হয়। ইন্টারভিউতে বলো: “বড় টেক কোম্পানিগুলো I/O ও স্কেলেবিলিটির জন্য Node ব্যবহার করে।”

---

### ১৪১. HTTPS সার্ভার কিভাবে বানাব?
**উত্তর:** `https` মডিউল + SSL সার্টিফিকেট ও কী ফাইল দরকার।  
`const https = require('https'); const fs = require('fs'); const options = { key: fs.readFileSync('key.pem'), cert: fs.readFileSync('cert.pem') }; https.createServer(options, (req, res) => { res.writeHead(200); res.end('Hello'); }).listen(443);`  
লোকালে সেলফ-সাইনড সার্টিফিকেট বানাতে `openssl` ব্যবহার করা হয়।

---

### ১৪২. request মডিউল (NPM) কি?
**উত্তর:** আগে HTTP ক্লায়েন্ট হিসেবে ব্যাপক ব্যবহার হতো; এখন **deprecated**। অন্য সার্ভার/API কল করতে `request(url, callback)` ব্যবহার করা হতো। বদলে **axios** বা **node-fetch** / নেটিভ **fetch** (Node 18+) ব্যবহার করো। কনসেপ্ট: Node দিয়ে ক্লায়েন্ট বানিয়ে অন্য সার্ভার থেকে ডাটা নেওয়া।

---

### ১৪৩. MEAN Stack কি?
**উত্তর:** চারটা টেকনোলজির সমন্বয়: **M**ongoDB (ডাটাবেস), **E**xpress (ওয়েব ফ্রেমওয়ার্ক), **A**ngular (ফ্রন্টএন্ড ফ্রেমওয়ার্ক), **N**ode.js (সার্ভার)। পুরো স্ট্যাকেই জাভাস্ক্রিপ্ট (অ্যাংগুলার টাইপস্ক্রিপ্ট)। ফুল-স্ট্যাক জাভাস্ক্রিপ্ট ডেভেলপমেন্ট ও এক ল্যাঙ্গুয়েজে ফ্রন্ট-ব্যাক এগিয়ে যাওয়ার জন্য MEAN ব্যবহার হয়। MERN = Angular এর জায়গায় React।

---

### ১৪৪. Node.js এ Web Scraping কি?
**উত্তর:** সার্ভার-সাইড ওয়েব অ্যাপ থেকে প্রোগ্রাম্যাটিকভাবে ডাটা বের করা। ধাপ: URL থেকে HTML নেওয়া (axios/request), DOM পার্স করা (Cheerio বা jsdom), প্রয়োজনীয় নোড সিলেক্ট করে ডাটা এক্সট্রাক্ট, JSON/ডাটাবেসে সেভ। স্ক্র্যাপিং নীতিমালা ও robots.txt মেনে সতর্কভাবে ব্যবহার করতে হয়।

---

### ১৪৫. MQTT ও Node.js এ ব্যবহার?
**উত্তর:** MQTT হালকা মেসেজিং প্রোটোকল, IoT এ 많이 ব্যবহার। **Publisher** মেসেজ পাঠায়, **Subscriber** টপিক সাবস্ক্রাইব করে মেসেজ পায়। Node এ `mqtt` প্যাকেজ: ব্রোকারে কানেক্ট করে `client.subscribe('topic')`, `client.publish('topic', message)`। রিয়েল-টাইম সেন্সর ডাটা, নোটিফিকেশন ইত্যাদিতে ব্যবহৃত।

---

### ১৪৬. Node.js এ JSON ফাইল কিভাবে হ্যান্ডেল করব?
**উত্তর:** পড়া: `const data = JSON.parse(fs.readFileSync('file.json', 'utf8'));`। অ্যাসিঙ্ক: `fs.readFile('file.json', 'utf8', (err, str) => { const data = JSON.parse(str); });`। লেখা: `fs.writeFileSync('file.json', JSON.stringify(obj, null, 2));`। কনফিগ বা ক্রেডেনশিয়াল রাখতে JSON ফাইল ব্যবহার করা সাধারণ; পার্স করার পর অবজেক্ট হিসেবে ব্যবহার করো।

---

### ১৪৭. Visitor Counter কিভাবে ইমপ্লিমেন্ট করব?
**উত্তর:** একটা গ্লোবাল ভেরিয়েবল বাড়িয়ে প্রতিটি রিকোয়েস্টে কাউন্ট বাড়ানো। উদাহরণ: `let hits = 0; server = http.createServer((req, res) => { hits++; res.writeHead(200); res.end('Visits: ' + hits); });`। প্রডাকশনে এই ভেরিয়েবল প্রসেস রিস্টার্টে রিসেট হয়; স্থায়ী কাউন্ট চাইলে Redis বা DB ব্যবহার করো।

---

### ১৪৮. Node.js এর মূল কম্পোনেন্টগুলো কি?
**উত্তর:** (১) **V8** — গুগলের জাভাস্ক্রিপ্ট ইঞ্জিন, JS কে মেশিন কোডে কম্পাইল। (২) **Libuv** — সি/সি++ লাইব্রেরি, অ্যাসিঙ্ক I/O ও ইভেন্ট লুপ; ফাইল, নেটওয়ার্ক, টাইমার ম্যানেজ করে। (৩) **Node.js Core APIs** — fs, http, path ইত্যাদি বিল্ট-ইন মডিউল; ডিফল্টে অ্যাসিঙ্ক, সিনক্রোনাস ভার্সনও আছে। এই তিনটা মিলে Node রানটাইম।
